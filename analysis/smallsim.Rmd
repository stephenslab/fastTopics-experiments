...
In this first example, we simulate counts from a multinomial topic
model with 6 topics.
...
The topic proportions for each sample are randomly drawn from the
...
the simulated counts: the EM updates, and the sequential coordinate
descent (SCD) updates. We initialize the model fitting by first
running 50 EM updates so that the updates are more likely to be moving
toward the same local maximum of the Poisson NMF likelihood surface.
