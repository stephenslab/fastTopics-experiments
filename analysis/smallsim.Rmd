...
```{r loadings-scatterplot-1, fig.width=2.5, fig.height=2.5}
...
```{r loadings-scatterplot-2, fig.width=2.5, fig.height=2.5}
pdat <- data.frame(x = as.vector(fit1$L),y = as.vector(fit2$L))
ggplot(pdat,aes(x = x,y = y)) +
  geom_point(color = "white",fill = "black",shape = 21,size = 1.75) +
  geom_abline(intercept = 0,slope = 1,color = "skyblue",linetype = "dotted") +
...
axes show the log-likelihoods):

```{r likelihood-scatterplot, fig.width=3, fig.height=2.5}
